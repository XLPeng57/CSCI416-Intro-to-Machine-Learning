...
def train_one_vs_all(X, y, num_classes, lambda_val):
    ...
    Outputs:
      weight_vectors   matrix of weight vectors for each class 
                       ...
                       (2d array shape n x num_classes) 400 x 10
    ...
    for i in range(num_classes):
      weight_vector, intercept = train_logistic_regression(X, y==i, lambda_val)
      if i == 0:
        weight_vectors = weight_vector
        intercepts = intercept
      else:
        weight_vectors = np.column_stack((weight_vectors, weight_vector))
        intercepts = np.column_stack((intercepts, intercept))
...
def predict_one_vs_all(X, weight_vectors, intercepts):
    ...
    Inputs: 
      X                data matrix (2d array shape m x n) 4000 400
      weight_vectors   matrix of weight vectors for each class 
                       weight vector for class c in the cth column
                       (2d array shape n x num_classes) 400 10
      intercepts       vector of intercepts for all classes
                       (1d array length num_classes)   
    ...
    pred = X.dot(weight_vectors) + intercepts
    p = 1/(1 + np.exp(-pred))
...
def display_data(X, im_width=None, return_mosaic=False):
    ...
    m, n = X.shape # m400 n10 width=20 height=20
